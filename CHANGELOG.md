...
* Brought the in-app documentation reader to mobile: tapping a "Docs" resource link now opens the doc inside Toolbox (markdown body, breadcrumb context, an "On this page" section jump, tap-to-copy code blocks, and in-content doc-to-doc links) instead of ejecting to GitHub, at parity with desktop. The viewer's platform-neutral core (doc types, link/section utilities, a headless `DocViewModel`, and a shared `docContent` body component) was extracted to a new `core/docs` module that both the desktop and mobile clients build on - a reference example of mobile/desktop code reuse.
