...
* Redesigned the admin Grid test panel around the desktop app's example `Wrapper` - options now live in a documented sidebar rail (grouped, with per-option help and hover-revealed Hoist API names, plus disable cascades that always display effective values), primary actions moved to the grid toolbar, and a placeholder CTA fronts the first load. The benchmark dialog gained an orientation intro (including GC-environment guidance), an in-dialog config chooser, active-config capture on every result row, and grouped result columns.
