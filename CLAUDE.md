...
**CLI Tools** — In Toolbox the MCP server is always enabled, so prefer it; the CLI launchers exist
as a peer surface for shell-based workflows, scripts, and quick one-off checks from a terminal.
They're `bin` entries in the hoist-react `package.json` and are available via `npx` in any project
that depends on hoist-react. **You must run these from the `client-app/` directory** (where
hoist-react is installed as a dependency) — they will not resolve from the project root. Ensure
`yarn install` has been run in `client-app/` first.
...
services, and conventions across 20+ feature docs and upgrade notes. Two interfaces expose this
content; both share the same underlying registries and produce identical output:

**MCP Server (hoist-core)** — Use the `hoist-core-search-docs`, `hoist-core-list-docs`,
`hoist-core-read-doc`, `hoist-core-search-symbols`, `hoist-core-get-symbol`, and
`hoist-core-get-members` tools, plus `hoist-core://docs/{id}` resources for direct document access.

**CLI Tools** — In Toolbox the MCP server is always enabled, so prefer it; the CLI launchers exist
as a peer surface for shell-based workflows, scripts, and quick one-off checks from a terminal.
They're project-local launchers under `./bin/`, generated by `./gradlew installHoistCoreTools`
(re-run after cloning the repo or bumping `hoistCoreVersion`). Run from the project root:

```bash
# Documentation
./bin/hoist-core-docs search "BaseService lifecycle"   # Search all docs by keyword
./bin/hoist-core-docs read docs/base-classes.md        # Read a specific doc by ID
./bin/hoist-core-docs list                             # List all available docs
./bin/hoist-core-docs conventions                      # Print coding conventions
./bin/hoist-core-docs index                            # Print the documentation index

# Groovy/Java symbols
./bin/hoist-core-symbols search getOrCreate            # Search for symbols and members
./bin/hoist-core-symbols symbol BaseService            # Get details for a specific symbol
./bin/hoist-core-symbols members BaseService           # List all members of a class/interface
```

Run `./bin/hoist-core-docs --help` and `./bin/hoist-core-symbols --help` for full usage.
...
AST-based symbol search for classes, services, controllers, and their members.

The server runs via a project-local launcher (`./bin/hoist-core-mcp`) generated by the
`installHoistCoreTools` Gradle task. That task resolves a fat JAR pinned to `hoistCoreVersion` from
`gradle.properties` and writes launchers under `bin/` (gitignored). The MCP runs in bundled mode,
reading docs and symbol data baked into the JAR — since Toolbox typically tracks a hoist-core
SNAPSHOT, that content stays very close to `develop`. After cloning the repo or bumping
`hoistCoreVersion`, run `./gradlew installHoistCoreTools` once to (re)generate the launchers.
Requires Java 17+.

The same install task also writes `./bin/hoist-core-docs` and `./bin/hoist-core-symbols` CLI
launchers, exposing identical content for shell-based use or environments without MCP support.
