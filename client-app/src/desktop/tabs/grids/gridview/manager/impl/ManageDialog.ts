...
export const manageDialog = hoistCmp.factory<ManageDialogModel>({
    ...
    render({model}) {
        return dialog({
            ...
            className: 'tb-gridview-mgr__manage-dialog',
...
const gridPanel = hoistCmp.factory({
    render() {
        return panel({
            className: 'tb-gridview-mgr__manage-dialog__grid-panel',
...
const formPanel = hoistCmp.factory<ManageDialogModel>({
    render({model}) {
        const {selectedId, noun, pluralNoun, formModel, canEdit} = model,
            {values} = formModel,
            aclField = model.gridModel.store.getField('acl') as any,
            aclLookup = aclField.lookup;
        ...
        return panel({
            className: 'tb-gridview-mgr__manage-dialog__form-panel',
            item: vframe({
                className: 'tb-gridview-mgr__manage-dialog__form',
                items: [
                    form({
                        fieldDefaults: {
                            inline: true,
                        ...
                        items: [
                            ...
                            formField({
                                ...
                                item: select({
                                    enableClear: true,
                                    enableMulti: true,
                                    options: aclLookup.roles,
                                    placeholder: 'Restrict to one or more app roles...'
                                }),
                                disabled: model.willBeGlobal,
                                omit: !model.showRoles
                            })
                        ]
                    }),
                    spacer({
                    ...
                    div({
                        items: [
                            `Created ${fmtCompactDate(values.dateCreated)} by ${
                                values.createdBy === XH.getUsername() ? 'you' : values.createdBy
                            }.`,
                            br(),
