...
export const saveDialog = hoistCmp.factory<SaveDialogModel>({
    ...
    render({model}) {
        const {viewStub, isAdd, parentModel} = model;
        return dialog({
            isOpen: true,
            icon: isAdd ? Icon.plus() : Icon.copy(),
            title: isAdd
                ? `Create new ${parentModel.capitalNoun}`
                : `Save "${viewStub.name}" as...`,
            className: 'tb-gridview-mgr__save-as-dialog',
...
const formPanel = hoistCmp.factory<SaveDialogModel>({
    render({model}) {
        return panel({
            item: vframe({
                className: 'tb-gridview-mgr__save-as-dialog__form',
                items: [
                    form({
                        fieldDefaults: {
                            inline: true,
