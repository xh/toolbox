    ...
    render({model}) {
        ...
        return wrapper({
            description: [
                ...
                p('Given a URL such as ', a({href: routedUrl, item: routedUrl}), ', where 123 is a record ID, we can auto-select the matching record in the grid. Updates to application state can be pushed back to the URL - try selecting a different record in the grid and observe the URL change.'),
                p('Note that this routing relies on an appropriate route path being defined in the config returned by AppModel.getRoutes().')
            ],
            item: panel({
