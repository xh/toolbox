...
import {a, code, p} from '@xh/hoist/cmp/layout';
...
export const simpleRoutingPanel = hoistCmp.factory({
    ...
    render({model}) {
        ...
        return wrapper({
            description: [
                p('Hoist provides functionality for route parameters to interact with UI components. The grid below has its selected record synced with a routable URL.'),
                p('Given a URL such as ', a({href: routedUrl, item: routedUrl}), ', where ', code('123'), ' is a record ID, we can auto-select the matching record in the grid. Updates to application state can be pushed back to the URL - try selecting a different record in the grid and observe the URL change.'),
                p('Note that this routing relies on an appropriate route path being defined in the config returned by ', code('AppModel.getRoutes()'), '.')
            ],
            item: panel({
