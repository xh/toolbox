        ...
        return wrapper({
            ...
            description:
                [
                    ...
                    'components. The grid below has its selected record synced with a routable',
                    'URL.',
                    '',
                    'Given a URL such as ['
                ] +
                ...
                '](' +
                routedUrl +
                '), where 123 is a record ID, we can auto-select the matching record in the grid. Updates to application state can be pushed back to the URL - try selecting a different record in the grid and observe the URL change.\n\nNote that this routing relies on an appropriate route path being defined in the config returned by `AppModel.getRoutes()`.',
            links: [
                {
