...
/** WebSocket topic the server (02-02 `DataLabPushService.TOPIC`) pushes update diffs on. */
const TOPIC = 'xhDataLab/updates';

/**
 * Client-side WebSocket-push delivery adapter for the Data Lab measurement harness.
 *
 * This is the WS-push twin of {@link HttpIngestAdapter}: it carries the SAME diff shape into the
 * SAME two-op ingest contract - only the delivery differs (the clean `transport` knob). The harness
 * core fetches nothing; this adapter owns the WebSocket transport. The server pushes diffs on the
 * dedicated `xhDataLab/updates` topic; this adapter subscribes, buffers them, and exposes a
 * pull-style `nextDiffAsync()` so the harness's injected per-iteration callback works identically
...
export class WebSocketIngestAdapter {
    ...
    /** Buffer of diffs received from the socket but not yet consumed by the harness. */
    ...
    private subscribe() {
        ...
        this.subscription = XH.webSocketService.subscribe(TOPIC, msg => this.onDiff(msg));
    }
    ...
    /** Handle one pushed diff - hand it straight to a waiting consumer, else buffer it. */
    private onDiff(msg: PlainObject) {
